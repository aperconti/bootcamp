...
```{r}
mean_reading_score <- summarize(students, mean(reading_score)) %>% round(2)
...
prct_passing_reading <- filter(students, reading_score > 70) %>% nrow()*100/student_count
prct_passing_reading <- round(prct_passing_reading,2)

paste(prct_passing_reading, "% of the students have passing reading scores.")
```
