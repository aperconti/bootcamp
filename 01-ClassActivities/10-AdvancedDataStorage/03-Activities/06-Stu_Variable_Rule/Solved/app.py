...
def welcome():
    return (
        ...
        f"<a href='/api/v1.0/justice-league'>All Heroes</a><br/>"
        f"<a href='/api/v1.0/justice-league/superhero/batman'>Who is Batman?</a>"
    )
...
def justice_league_by_superhero__name(key, value):
    """Fetch the Justice League character that matches
       the path variable supplied by the user, or a 404 if not."""
    characters = []

    for character in justice_league_members:
        canonicalized = value.replace(" ", "").lower()
        search_term = character[key].replace(" ", "").lower()
        if search_term == canonicalized:
