...
var selection = ul.selectAll("li") // creates virtual selection
  .data(arr) // binds data
  .join("li") // appends li element for each item in array (since there are currently none)
  .text(d => d); // sets the text of each element to match the items in array
...
/* Bonus - modify the code to set the text of each element to 
"<index in the array>: <item from the array>" */
// Be sure to comment out your number 4 (not the arr or ul variables) before running your bonus code.
// // YOUR CODE HERE //
var selection = ul.selectAll("li") // creates virtual selection
  .data(arr); // binds data
selection.join("li") // appends li element for each item in array (since there are currently none)
